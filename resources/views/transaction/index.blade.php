...
<table class="table table-striped table-condensed table-hover" id="tblTxnList">
    ...
    <tr>
        <td>{!! $txn->id !!}</td>
        <td>{!! $txn->contact->lastname ? $txn->contact->lastname .', ' : '' !!}{!! $txn->contact->firstname or ''!!}</td>
        <td>{!! $txn->transaction_no !!}</td>
        <td>{!! $txn->name !!}</td>
        <td>{!! '$'.$txn->amount !!}</td>
        <td>{!! App\Models\Transaction::$_paymentMode[$txn->payment_mode] !!}</td>
        <td>{!! App\Models\Transaction::$_paymentType[$txn->payment_mode][$txn->payment_type] !!}</td>
